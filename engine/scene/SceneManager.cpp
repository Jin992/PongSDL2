...
namespace Engine {
    namespace Scene {
        ...
        void SceneManager::add_scene(const std::string &scene_name, engine_scene_p scene, std::string &err) {
            auto search_res = _scenes.find(scene_name);
            if (search_res != _scenes.end()) {
                err = std::string(scene_name + " scene already exist");
                return;
            }
            _scenes.insert(std::pair<std::string, engine_scene_p>{scene_name, scene});
        }
        ...
        engine_scene_p SceneManager::current_scene(std::string &err) {
            if (!_init_scene) {
                err = std::string("Startup scene is not set, set startup scene.");
                return nullptr;
            }
